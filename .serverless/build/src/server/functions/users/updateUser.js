...
// src/server/functions/users/updateUser.ts
...
// src/app/modules/users/schemas/CreateUser.schema.ts
...
// src/app/modules/users/schemas/DeleteUser.schema.ts
var DeleteUserSchema = z.object({
  userId: z.string(),
  email: z.string().email()
});

// src/app/modules/users/schemas/UpdateUser.schema.ts
...
// src/app/modules/users/schemas/Me.schema.ts
...
// src/app/modules/users/controllers/update-user/UpdateUser.controller.ts
...
var UpdateUserController = class {
  ...
  async handle(request) {
    try {
      ...
      if (error instanceof import_client_cognito_identity_provider4.UsernameExistsException) {
        return {
          statusCode: 409,
          body: {
      ...
      if (error instanceof import_client_cognito_identity_provider4.ResourceNotFoundException) {
        return {
          statusCode: 404,
          body: {
      ...
      if (error instanceof import_client_cognito_identity_provider4.NotAuthorizedException) {
        return {
          statusCode: 401,
          body: {
...
// src/app/modules/users/controllers/me/Me.controller.ts
var import_client_cognito_identity_provider5 = require("@aws-sdk/client-cognito-identity-provider");

// src/app/modules/users/services/update-user/UpdateUser.service.ts
...
var UsersRepository = class {
  ...
  async delete(dto) {
    const cognitoCommand = new import_client_cognito_identity_provider6.AdminDeleteUserCommand({
      UserPoolId: process.env.COGNITO_USER_POOL_ID,
      Username: dto.email
    });
    await this.cognitoClient.send(cognitoCommand);
    const dynamoCommand = new import_lib_dynamodb.DeleteCommand({
      TableName: "WaiterAppTable",
      Key: {
  ...
  async getLoggedUser(dto) {
    const cognitoCommand = new import_client_cognito_identity_provider6.AdminGetUserCommand({
      Username: dto.id,
      UserPoolId: process.env.COGNITO_USER_POOL_ID
    });
    ...
    const dynamoCommand = new import_lib_dynamodb.QueryCommand({
      TableName: "WaiterAppTable",
      IndexName: "GSI1PK-GSI1SK-index",
      KeyConditionExpression: "#GSI1PKEY = :pkey and #GSI1SKEY = :skey",
      ExpressionAttributeValues: {
        ":pkey": "USERS",
        ":skey": addPrefix("user", UserAttributes[0].Value)
      },
    ...
    const { Items } = await this.dynamoClient.send(dynamoCommand);
    if (!Items) {
      return {};
    }
    return {
  ...
  async getUserById(dto) {
    const dynamoGetCommand = new import_lib_dynamodb.GetCommand({
      TableName: "WaiterAppTable",
      Key: {
    ...
    const { Item } = await this.dynamoClient.send(dynamoGetCommand);
    return Item;
  }
  async update(dto) {
    const cognitoCommand = new import_client_cognito_identity_provider6.AdminUpdateUserAttributesCommand({
      Username: dto.email,
      UserPoolId: process.env.COGNITO_USER_POOL_ID,
      UserAttributes: [
        {
    ...
    await this.cognitoClient.send(cognitoCommand);
    const dynamoPutCommand = new import_lib_dynamodb.UpdateCommand({
      TableName: "WaiterAppTable",
      Key: {
        PK: addPrefix("user", dto.id),
        SK: addPrefix("user", dto.id)
...
// src/factories/repositories/users/makeUsersRepository.factory.ts
function makeUsersRepository() {
  return new UsersRepository(cognitoClient, dynamoClient);
}

// src/factories/services/users/makeUpdateUserService.factory.ts
function makeUpdateUserService() {
  return new UpdateUserService(makeUsersRepository());
}

// src/factories/controllers/users/makeUpdateUserController.factory.ts
