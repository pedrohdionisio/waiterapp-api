...
// src/server/functions/auth/signIn.ts
var signIn_exports = {};
...
var SignInController = class {
  ...
  async handle(request) {
    try {
      ...
      if (error instanceof import_client_cognito_identity_provider.UserNotFoundException) {
        return {
          statusCode: 404,
          body: {
      ...
      if (error instanceof import_client_cognito_identity_provider.InvalidParameterException || error instanceof import_client_cognito_identity_provider.NotAuthorizedException) {
        return {
          statusCode: 401,
          body: {
...
// src/app/modules/auth/services/sign-in/SignIn.service.ts
var SignInService = class {
  constructor(authRepository) {
    this.authRepository = authRepository;
  }
  async execute(input) {
    const { accessToken, refreshToken } = await this.authRepository.login(input);
    return {
      accessToken,
      refreshToken
    };
...
// src/factories/services/auth/makeSignInService.factory.ts
function makeSignInService() {
  return new SignInService(makeAuthRepository());
}

// src/factories/controllers/auth/makeSignInController.factory.ts
